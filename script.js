...
function formatHeroTitle(text) {
  ...
  if (words.length === 2) return words[0] + ' <span class="serif-accent">' + words[1] + '</span>';
  return words[0] + ' <span class="serif-accent">' + words[1] + '</span> ' + words.slice(2).join(' ');
}
...
function renderPageContent() {
  ...
  if (aboutTitleEl) aboutTitleEl.innerHTML = (c.aboutTitle || DEFAULT_PAGE_CONTENT.aboutTitle) + ' <span class="serif-accent">POIG</span>';
...
function renderActivities() {
  ...
  if (!grid) return;
  const items = [...siteData.activities].sort((a, b) => (b.date || '').localeCompare(a.date || ''));
  grid.innerHTML = items.map(a => activityCardHTML(a)).join('');

  document.querySelectorAll('[data-expandable]').forEach(card => {
    card.addEventListener('click', e => {
      if (e.target.closest('.activity-photo, a, button, input, textarea, select')) return;
...
function activityCardHTML(a) {
  ...
  return `
    <div class="activity-card" data-expandable>
      ...
      <div class="activity-details">
        ...
        ${a.photos && a.photos.length ? `
          <div class="activity-photos">
            ${a.photos.map((p, idx) => { const src = (a.photosData && a.photosData[idx]) ? a.photosData[idx] : p; return `<img src="${src}" class="activity-photo" alt="" loading="lazy" onerror="this.style.display='none'">`; }).join('')}
          </div>
        ` : ''}
      </div>
    </div>
...
function renderEvents() {
  ...
  if (!items.length) {
    grid.innerHTML = `
      <div class="event-card" style="grid-column:1/-1;max-width:550px;margin:0 auto;">
...
function renderNews() {
  ...
  grid.innerHTML = items.map((n, i) => {
    ...
    return `
    <div class="news-card${i === 0 ? ' active' : ''}">
      ...
      <div class="news-card-body">
        ...
        ${n.extraPhotos && n.extraPhotos.length ? `
          <div class="activity-photos" style="margin-top:14px;">
